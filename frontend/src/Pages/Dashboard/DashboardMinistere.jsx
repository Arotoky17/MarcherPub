...
const Sidebar = ({ darkMode, toggleDarkMode, activeMenu, setActiveMenu }) => (
      ...
      <div className="mb-6">
        ...
        <nav className="space-y-1">
          <button
            ...
            className={`w-full text-left flex items-center space-x-3 px-3 py-2 rounded-lg ${
...
const DashboardMinistere = () => {
          ...
          <div>
            ...
            <div className="space-y-4">
              {users && users.length > 0 ? users.map(user => (
                ...
                <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Aucun utilisateur trouvé</p>
              )}
            </div>
          </div>
        );
          ...
          <div>
            ...
            <div className="space-y-4">
              {offers && offers.length > 0 ? offers.map(offer => (
                ...
                <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Aucune offre trouvée</p>
              )}
            </div>
          </div>
        );
      case 'applications':
        return (
          <div>
            ...
            <div className="space-y-4">
              {candidatures && candidatures.length > 0 ? candidatures.map(candidature => (
                <div key={candidature.id} className={`p-4 rounded-lg shadow ${darkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'}`}>
                  <div className="font-medium">Offre: {candidature.offerId}</div>
                  <div className="text-xs">Entreprise: {candidature.entrepriseId}</div>
                  <div className="text-xs">Statut: {candidature.status}</div>
                </div>
              )) : (
                <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Aucune candidature trouvée</p>
              )}
            </div>
          </div>
        );
  ...
  return (
    <div className={`flex min-h-screen ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <Sidebar darkMode={darkMode} toggleDarkMode={toggleDarkMode} activeMenu={activeMenu} setActiveMenu={setActiveMenu} />
