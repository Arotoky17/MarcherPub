...
exports.updateCandidatureStatus = async (req, res) => {
  ...
  let otherCandidatures = null; // Correction: initialiser la variable
  ...
  try {
    ...
    if (status === 'acceptée') {
      ...
      otherCandidatures = await Candidature.findAll({
        where: {
          offerId: candidature.offerId,
          status: 'en_attente',
    ...
    res.json({
      ...
      autoRejected: status === 'acceptée' && otherCandidatures ? otherCandidatures.length : 0 // Correction ici
    });
